...
class GetAppointments(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            datestart,dateend=get_week_range(week_num,today.year)
            datestart_shifted = datestart - timedelta(days=7)
            dateend_shifted = dateend + timedelta(days=7)
            machines = request.query_params.getlist('machines[]')
            if machines:
                appointments = Tempactiveplan.objects.filter(
                    machine__in=machines, 
                    starttime__gte=datestart_shifted,
                    starttime__lte=dateend_shifted,
                ).values(
...
class get_remain_plan_old(APIView):
    permission_classes = [ProductionPerm |PlannerPerm|PlantCoPerm| AdminPerm]
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            for plan in plan_data:
                ...
                if planweek in latest_versions_map and versionno == latest_versions_map[planweek]:
                    ...
                    result_data.append({
                        ...
                        'percentage' : round(((plancount - remain) / plancount) * 100, 2) if plancount else 0,
            ...
            for item in result_data:
                ...
                if matching_entries:
                    for key, done_entry in matching_entries.items():
                        ...
                        item['percentage'] = round(((item['plan'] - item['remain']) / item['plan']) * 100, 2) if item['plan'] else 0,
            ...
            done_dict_keyed=get_residue(done_dict.items())
            # Return the result
            return Response({
                'success': True,
                'data': result_data,
        ...
        try:
            ...
            for plan in plan_data:
                ...
                if planweek in latest_versions_map and versionno == latest_versions_map[planweek]:
                    ...
                    price_notice = ViewInfoPriceNov2024.objects.filter(material_no=zca).first()
                    if price_notice:  
                            price_unit_decimal = float(price_notice.price_unit.strip())
                            estimate_price += price_unit_decimal * float(plancount)
                            estimate_price_prod+=price_unit_decimal*float(goodcount)
                    # Calculate remaining counts and pallets
                    remain_ton_t = remain = plancount - goodcount
                    remain_pallet_t=remain_pallet = round(remain / pcsperpallet, 2) if pcsperpallet else 1
                    plan_pallet = round(plancount / pcsperpallet, 2) if pcsperpallet else 1
            ...
            # ดูของเครื่องอื่นเก็บตก week ที่แล้ว
            for item in result_data:
                remain=item['remain']

                if remain < 0 : # เดินเกินแผน
                #     #Check last week remain
                    if check_exist_lastweek(item):
                        item=check_lastweek_remain(item)
                        assign_other_result(result_data,item,True) 
                #         #Check last week both match
                    elif check_exist_lastweek_onematch(item):
                        #Check last week only zca
                        pass
                    elif check_exist_nextweek(item):   
                        item=check_next_week_remain(item)
                        assign_other_result(result_data,item,False) 
                #     #Check next week 
                #         #Check next week both match


                # elif remain > 0 : # เดินไม่ครบ
                #     if check_exist_nextweek_forpast(item):
                #         item=check_lastweek_remain(item)
                
                #     elif check_exist_pastweek_forff(item):   
                #         item=check_next_week_remain(item)
            ...
            'machine_done_ton': machine_done_ton,
            'done_dict': done_dict_keyed, 
            'estimate_price':int(estimate_price / 10**6 * 100) / 100,
            'estimate_price_prod':int(estimate_price_prod / 10**6 * 100) / 100,
            }, status=status.HTTP_200_OK)
...
def assign_other_result(result_data,assign_item,lastweek):
    zca=assign_item.get('materialcode')
    mc= assign_item.get('machine_plan')
    year_check=int(assign_item.get('planweek').split('_')[0])
    week_check=int(assign_item.get('planweek').split('_')[-1])
    week_assign = f"{year_check}_{week_check -1}"
    week_assign_future = f"{year_check}_{week_check +1}"
    for other_item in result_data:
        if lastweek:
            if other_item['planweek'] == week_assign and other_item['materialcode'] == zca and other_item['machine_plan'] == mc:
                other_item['next_week_taken'] = assign_item['last_week_taken']
                other_item['remain'] -= assign_item['last_week_taken']
                # other_item['remain_pallet']=other_item['remain']/other_item['`']
                other_item['percentage'] = round((other_item['plan'] - other_item['remain']) / other_item['plan'] * 100, 2)
        else:
            if other_item['planweek'] == week_assign_future and other_item['materialcode'] == zca and other_item['machine_plan'] == mc:
                other_item['last_week_taken'] = assign_item['next_week_taken']       
                other_item['remain'] -= assign_item['next_week_taken']
                other_item['percentage'] = round((other_item['plan'] - other_item['remain']) / other_item['plan'] * 100, 2)



def get_residue(done_dict):
    done_dict_keyed = []
    for key, value in done_dict:
        zca, machine, week = key  
        value['materialcode']=zca
        value['machine']=machine
        done_dict_keyed.append(value)
    return done_dict_keyed

def check_exist_lastweek(item):
    planweek=item['planweek']
    machine=item['machine_plan']
    zca=item['materialcode']
    year = planweek.split('_')[0]
    week = planweek.split('_')[1]
    lastweek = f"{year}_{int(week)-1}"

    largest_id_data = RemainPlan.objects.filter(
        planweek=lastweek,
        machine=machine,
        zca=zca
    ).order_by('-id').first()

    if largest_id_data and largest_id_data.remain_qty > 0: # ของขาด
        return True
    else:
        return False
    
def check_exist_lastweek_bfp(item):
    planweek=item['planweek']
    machine=item['machine_plan']
    zca=item['materialcode']
    year = planweek.split('_')[0]
    ...
    if largest_id_data and largest_id_data.remain_qty > 0:
        return True
    else:
        return False

def check_exist_lastweek_onematch(item):
    planweek=item['planweek']
    machine=item['machine_plan']
    zca=item['materialcode']
    year = planweek.split('_')[0]
...
def check_exist_nextweek(item):
    planweek=item['planweek']
    machine=item['machine_plan']
    zca=item['materialcode']
    year = planweek.split('_')[0]
    week = planweek.split('_')[1]
    nextweek = f"{year}_{int(week)+1}"
...
def check_exist_nextweek_forpast(item):
    planweek=item['planweek']
    machine=item['machine_plan']
    zca=item['materialcode']
    year = planweek.split('_')[0]
...
def check_exist_pastweek_forff(item):
    planweek=item['planweek']
    machine=item['machine_plan']
    zca=item['materialcode']
    year = planweek.split('_')[0]
...
def check_lastweek_remain(item):
    planweek=item['planweek']
    machine=item['machine_plan']
    zca=item['materialcode']
    year = planweek.split('_')[0]
    ...
    lastweek = f"{year}_{int(week)-1}"
    nextweek = f"{year}_{int(week)+1}"
    # Get the record with the largest id
    largest_id_data = RemainPlan.objects.filter(
        planweek=lastweek,
        machine=machine,
        zca=zca
    ...
    largest_id_data_nw = RemainPlan.objects.filter(
        planweek=nextweek,
        machine=machine,
        zca=zca
    ).order_by('-id').first()
    curr=item['remain']
    if check_exist_lastweek(item) and curr<0: # อาทิตที่แล้วขาด ปัจจุบันเกิน
        if abs(curr) <= largest_id_data.remain_qty: # ขาดมากกว่าเหลือ
            item['remain'] += abs(curr)
            item['remain_pallet'] =0
            item['last_week_taken'] = abs(curr)
            item['lastplan_week'] =lastweek
            item['percentage'] = round(((item['plan'] -  item['remain']) / item['plan']) * 100, 2)
        else :  # เหลือมากกว่าขาด
            item['remain'] += largest_id_data.remain_qty
            item['remain_pallet'] += largest_id_data.remain_pallet
            item['last_week_taken'] = largest_id_data.remain_qty
            item['lastplan_week'] =lastweek
            item['percentage'] = round(((item['plan'] -  item['remain']) / item['plan']) * 100, 2)
...
def check_next_week_remain(item):
    planweek=item['planweek']
    machine=item['machine_plan']
    zca=item['materialcode']
    year = planweek.split('_')[0]
    ...
    if check_exist_nextweek(item) and item['remain']<0:
        print(item['materialcode'],item['planweek'],item['remain'],largest_id_data.remain_qty)
        taken=0
        # if largest_id_data.remain_qty > 0 and largest_id_data.remain_qty > -1*item['remain']:
        #     taken=item['remain']

        if largest_id_data.remain_qty > 0 and largest_id_data.remain_qty < -1*item['remain']:
            taken=largest_id_data.remain_qty 

        item['remain'] += taken
        item['next_week_taken'] = taken
        item['nextplan_week'] =nextweek
        item['percentage'] = round(((item['plan'] - item['remain']) / item['plan']) * 100, 2)
        if item['materialcode']=='ZCAWA110100013':
            print(item)
    return item
...
class get_operator_pef(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            zca_list = [item['zca_on'] for item in performance_data]
            price_notice_dict = {
                obj.material_no: float(obj.price_unit.strip())
                for obj in ViewInfoPriceNov2024.objects.filter(material_no__in=zca_list)
            }
            ...
            for item in performance_data:
                ...
                price_unit_decimal = price_notice_dict.get(zca_on)
                estimate_price = 0
                if price_unit_decimal:
                    estimate_price = price_unit_decimal * float(qty_loss)
                ...
                operator_performance[operator]['total_cost'] += estimate_price
...
class get_operator_past(APIView):
    permission_classes = [ProductionPerm | PlannerPerm | PlantCoPerm | AdminPerm]

    def get(self, request, *args, **kwargs):
        try:

            operator_no = request.query_params.getlist('operator_id')
       
            performance_data = ViewWmsListfillplanproduction.objects.filter(
                operator_keyin=operator_no[0],
                approve_fill='success',
            ).values(
                'operator_keyin',  # Group by operator_keyin
                'machine'          # Group by machine
            ).annotate(
                total_qty_good=Sum('qty_good'),
                total_qty_loss=Sum('qty_loss')
            ).order_by('operator_keyin', 'machine')
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            Pastproduct = ViewPisMergereportProduction.objects.filter(**filter_params).annotate(
                ...
                total_lossweight=Sum('rejectweight'),
            ...
            formatted_data_reject = {zca: {period: 0 for period in all_periods} for zca in zca_list}
            formatted_data_reject_ton = {zca: {period: 0 for period in all_periods} for zca in zca_list}
            ...
            for item in Pastproduct:
                ...
                total_lossweight = item['total_lossweight'] if item['total_lossweight'] is not None else 0
                total_loss = item['total_loss'] if item['total_loss'] is not None else 0
                formatted_data[materialcode][period] = total_goodcount
                formatted_data_ton[materialcode][period] = total_goodweight
                formatted_data_reject_ton[materialcode][period] = total_lossweight
                formatted_data_reject[materialcode][period] = total_loss
            ...
            final_data_reject = {
                materialcode: [{'x': period, 'y': count} for period, count in periods.items()]
                for materialcode, periods in formatted_data_reject.items()
            }

            final_data_reject_ton = {
                materialcode: [{'x': period, 'y': count} for period, count in periods.items()]
                for materialcode, periods in formatted_data_reject_ton.items()
            }
            ...
            return Response({
                ...
                'Pastproductton_reject':final_data_reject,
                'Pastproductton_reject_ton':final_data_reject_ton,
            })
